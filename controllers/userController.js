...
export const register = catchAsyncError(async (req, res, next) => {
    ...
    sendToken(res, user, "Regiserd Successfully", 201);
});
...
export const login = catchAsyncError(async (req, res, next) => {
    ...
    if (!isMatch)
        return next(new ErrorHandler("Incorrect Email or Password", 401));
    sendToken(res, user, `Welecome back, ${user?.name}`, 200);
});
...
export const updateProfilePictures = catchAsyncError(async (req, res, next) => {
    // cloudinary : TODO
    const file = req.file;
...
export const addtoPlaylist = catchAsyncError(async (req, res, next) => {
    ...
    if (!course)
        return next(new ErrorHandler("Invalid Cource Id", 404));
...
User.watch().on("change", async () => {
    const stats = await Stats.find({}).sort({ cretedAt: "desc" }).limit(1);
